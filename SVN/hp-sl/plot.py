...
def plot(x_axis, y_axis, title, x_label, y_label, legend, type='linear'):
    ...
    for i in range(0, len(x_axis)):
        ...
        xint = range(min(x_axis[i]) - 1, ceil(max(x_axis[i]))+1)
        plt.xticks(xint)
...
def get_graphs(tech, num_threads, x_axis, y_axis_perf, y_axis_scal, y_axis_eff):
    ...
    data = []
    if tech == 'hybrid':
        serial_data = get_data('./out_serial')
    else:
    ...
    y_axis_eff += [[(data[i][1] / serial_data[1]) / x_axis[-1][i] for i in range(0, len(data))]]
...
def main():
    ...
    plot(x_axis, y_axis_perf, 'Performance - hp-sl.q', 'Number of threads', 'Number of games simulated', techs)
    plot(x_axis, y_axis_scal, 'Scalability (weak scaling) - hp-sl.q', 'Number of threads', 'Speedup', techs)
    plot(x_axis, y_axis_eff, 'Efficiency - hp-sl.q', 'Number of threads', 'Efficiency', techs)
